...
const input = `............................................411.....................363..134.........463.775..........................506...................
...
// A symbol is any character that's not a digit or a period (create a helper function for this)
// Could keep the prev, curr, and next indexes in a variable, then access like a multimensional array
// While curr...
//   If prev, ...
//   If next, ...

const isSymbol = (input) => input.length > 0 && Number.isInteger(+input) === false && input !== ".";

// Helper function for: Found an intersection? Great, go left and right and get the full number
//   Find the lower and upper bounds of the substring indexes and then get the substring and convert to a number and return
// Input: text: "467..114..", index: 2
const getStartAndEndOfNumberFromIndex = (text, index) => {
	let start = index, end = index;
	if (!isNaN(text[start])) {
		while (start > 0 && !isNaN(text[start - 1])) start--;
		while (end < text.length - 1 && !isNaN(text[end + 1])) end++;
	}
	return { start, end, key: `${start}-${end}` };
}
getStartAndEndOfNumberFromIndex("467..114..", 2);

const getNumberFromStringIndexes = (text, start, end) => +text.substring(start, end + 1) || 0

// Set prevIndex to -1, currIndex to 0, and nextIndex to 1
// For each line
//   Init pos to zero
//   For each character
//     If character is not a symbol, continue
//     Check prev row
//       Helper function to get the start and end of the number from the index
//     Check curr row
//     Check next row
//     Increment pos
...
const part1 = () => {
	...
	let prev = -1, curr = 0, next = 1;
	for (const line of lines) {
		for (let i = 0; i < line.length; i++) {
			if (!isSymbol(line[i])) continue;

			// Top
			if (prev >= 0) {
				const set = new Set();
				if (i >= 0) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[prev], i - 1)
					set.add(key);
				} {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[prev], i)
					set.add(key);
				} if (i < lines[prev].length) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[prev], i + 1)
					set.add(key);
				}
				for (const key of set) {
					const [start, end] = key.split("-")
					const number = getNumberFromStringIndexes(lines[prev], +start, +end)
					total += number;
				}
			}
			...
			{
				if (i >= 0) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[curr], i - 1)
					const number = getNumberFromStringIndexes(lines[curr], +start, +end)
					total += number;
				}
				if (i < lines[curr].length) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[curr], i + 1)
					const number = getNumberFromStringIndexes(lines[curr], +start, +end)
					total += number;
				}
			...
			if (next < lines.length) {
				const set = new Set();
				if (i >= 0) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[next], i - 1)
					set.add(key);
				} {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[next], i)
					set.add(key);
				} if (i < lines[next].length) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[next], i + 1)
					set.add(key);
				}
				for (const key of set) {
					const [start, end] = key.split("-")
					const number = getNumberFromStringIndexes(lines[next], +start, +end)
					total += number;
				}
			}
		}
...
// ================== PART 2 ==================
const getNumberFromStringIndexes2 = (text, start, end) => +text.substring(start, end + 1) || 1
	...
	for (const line of lines) {
		for (let i = 0; i < line.length; i++) {
			...
			if (prev >= 0) {
				...
				for (const key of set) {
					const [start, end] = key.split("-")
					const number = getNumberFromStringIndexes2(lines[prev], +start, +end)
			...
			// Middle
			{
				if (i >= 0) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[curr], i - 1)
					const number = getNumberFromStringIndexes2(lines[curr], +start, +end)
					numbersToMultiply.push(number)
				}
				if (i < lines[curr].length) {
					const { start, end, key } = getStartAndEndOfNumberFromIndex(lines[curr], i + 1)
			...
			if (next < lines.length) {
				...
				for (const key of set) {
					const [start, end] = key.split("-")
					const number = getNumberFromStringIndexes2(lines[next], +start, +end)
					numbersToMultiply.push(number)
				}
			}

			// Multiply the numbers
			numbersToMultiply = numbersToMultiply.filter(x => x !== 1)
			if (numbersToMultiply.length > 1) {
				const subtotal = numbersToMultiply.reduce((acc, factor) => acc * factor, 1)
				total += subtotal
			}
		}
		prev++; curr++; next++;
