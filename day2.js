...
const input = `Game 1: 3 blue, 7 green, 10 red; 4 green, 4 red; 1 green, 7 blue, 5 red; 8 blue, 10 red; 7 blue, 19 red, 1 green
...
// Init a total variable to 0
// For each line...
  // Replace "Game " with ""
  // Split the line by colon - left side is ID, right side is each game
  // Init a handfuls variable splitting the line by semicolon
  // Split by commas to get amount and colors
  // For each handful...
    // Trim the handful
    // Split by handful by spaces (left is amount, right is color)
    // See if the amount is greater than the max value
    // If it is, set a flag for invalid game to true and break from the loop
  // If invalid game is true, continue
  // Add the ID as a number to the total
// Return the total
...
const part1 = () => {
  ...
  for (const line of lines) {
    ...
    const [id, game] = line.replace("Game ", "").split(":");
    const handfuls = game.split(";");
    for (const handful of handfuls) {
      const colorsAndCounts = handful.trim().split(",");
      for (const colorAndCount of colorsAndCounts) {
        const [amount, color] = colorAndCount.trim().split(" ");
        if (+amount > max[color]) {
          invalidGame = true;
          break;
        }
...
// Init a total variable to 0
// For each line...
  // Init a maxGame map with keys of colors and values of counts
  // Replace "Game " with ""
  // Split the line by colon - left side is ID, right side is each game
  // Init a handfuls variable splitting the line by semicolon
  // Split by commas to get amount and colors
  // For each handful...
    // Trim the handful
    // Split by handful by spaces (left is amount, right is color)
    // Set that amount as the max for the current game
  // Multiply the game's values to get the power
...
const part2 = () => {
  ...
  for (const line of lines) {
    ...
    for (const handful of handfuls) {
      ...
      for (const colorAndCount of colorsAndCounts) {
        const [amount, color] = colorAndCount.trim().split(" ");
        maxGame[color] = Math.max(+amount, maxGame[color] || 0);
      }
    }
    let power = 1;
    for (const amount of Object.values(maxGame)) {
      power *= amount;
    }
    total += power;
  }
