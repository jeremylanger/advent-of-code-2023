...
// Init a total var to 0
// Loop through lines
// Init a first and last var to null
// Loop through each character in the string
// If it's not a number, continue
// If the first var is empty, assign
// If not, assign to last
// Put the two variables into a string
...
// Loop through lines
// Init a first and last var to null
// Init a tmp variable that will take in letters one at a time
// Loop through each character in the string
// Append current letter to string
// If it contains 'one', 'two', etc.
// Map it two the numeric value
// Assign numeric value to var called number
// Reset tmp to empty string
// If the current letter is numeric
// Assign numeric value to var called number
// If the first var is empty, assign
// If not, assign to last
// Put the two variables into a string
...
const numberNames = Object.keys(map);

// Part 2
const getNumberAtEndOfString = (haystack) => {
	for (const numberName of numberNames) {
		if (haystack.endsWith(numberName)) {
			return map[numberName];
		}
	}
	return null;
}
...
const part2 = () => {
	...
	const lines = input.split("\n");
	for (const line of lines) {
		let first = null, last = null, haystack = '';
		for (const letter of line) {
			haystack += letter;
			let number = null;

			if (Number.isInteger(+letter)) number = +letter;

			if (!number) number = getNumberAtEndOfString(haystack);
